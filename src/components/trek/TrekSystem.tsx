...
export default function TrekSystem() {
  ...
  const sections = useMemo(
    () => getSections(state, BASE_SECTIONS),
    [state.extra],
  );
  const totals = useMemo(
    () => calculateTotals(sections, state),
    [sections, state],
  );
  ...
  return (
    <div className="trek" ref={root}>
      <Hud totals={totals} />
      <ProfileBar state={state} setState={setState} />
      <main className="wrap">
      ...
      {message ? <Toast message={message} onDone={clearMessage} /> : null}
    </div>
  );
}
