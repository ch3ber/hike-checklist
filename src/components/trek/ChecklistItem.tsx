...
export function ChecklistItem({ item, custom, state, setState }: ChecklistItemProps) {
  ...
  const toggleDiscarded = () => {
    setState((current) => {
      ...
      return {
        ...current,
        off: nextOff,
        chk: current.off[item.id] ? current.chk : { ...current.chk, [item.id]: false },
      }
    })
  }
  ...
  const remove = () => {
    ...
    setState((current) => {
      const omit = <T,>(record: Record<string, T>) =>
        Object.fromEntries(Object.entries(record).filter(([id]) => id !== item.id))
      return {
        ...current,
